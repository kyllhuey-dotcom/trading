...
def test_scan_timeout_longer_than_lock_stale():
    """P0: SCAN_ALL_TIMEOUT_S > SCAN_LOCK_STALE_S so a full scan is never
    treated as stuck/re-entered (lock stale must stay above a single scan)."""
    assert idx.SCAN_ALL_TIMEOUT_S > idx.SCAN_LOCK_STALE_S
    assert idx.SCAN_ALL_TIMEOUT_S >= 600


def test_scan_constants_defined():
    """Audit (2026-08-23): lock stale 180s, scan timeout 600s."""
