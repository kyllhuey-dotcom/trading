...
bot_state = {
    ...
    "is_running": False, "latest_scan": [], "active_trades": [],
    "engine_stats": {"markets": 0, "scanned": 0, "signals": 0, "tradable": 0},
    "selected_market": "btc_usdt"
}

metrics_state = {"total_scans": 0, "total_trades": 0, "total_errors": 0, "signals_by_strategy": {"structure": 0, "arbitrage": 0, "tape": 0, "liquidity": 0}, "start_time": datetime.now()}
...
# --- 3. THE MICRO-LOOPS (Institutional Core) ---
...
async def tick_capital():
    async with state_lock:
        ...
        if mode == "REAL":
            ...
            total = sum(b.get("total_usdt", 0.0) for b in balances.values() if "total_usdt" in b)
        ...
        safety = risk_engine.check_global_safety(bot_state["equity"], portfolio_engine.get_daily_pnl(mode))
        if not safety["safe"]:
            logger.error(f"GLOBAL RISK LIMIT: {safety['reason']}"); await emergency_stop_logic()
...
async def tick_scanner():
    if bot_state["is_running"]:
        ...
        async with state_lock:
            ...
            bot_state["engine_stats"].update({"markets": len(results), "scanned": len([r for r in results if r.get("status") == "LIVE"]), "signals": len([r for r in results if r.get("score", 0) >= 80]), "tradable": len([r for r in results if r.get("tradable")])})
        if bot_state["armed"]:
            for res in results:
                if res.get("tradable") and len(bot_state["active_trades"]) < 10:
                    if any(p["symbol"] == res["symbol"] for p in bot_state["active_trades"]): continue
                    ticker = await data_engine.fetch_ticker(res["symbol"])
                    sig = res.get("signal_data")
                    risk_data = risk_engine.calculate_position_size(bot_state["balance"], sig["entry"], sig["sl"], sig["direction"], symbol=res["symbol"], active_positions=bot_state["active_trades"])
                    if risk_data.get("allowed"):
                        exec_res = await execution_router.execute(bot_state["mode"], sig, risk_data, ticker)
                        if exec_res.get("success"):
                            metrics_state["total_trades"] += 1; strat = sig.get("strategy", "structure"); metrics_state["signals_by_strategy"][strat] += 1
                            asyncio.create_task(notification_engine.notify("ORDER_OPEN", {"symbol": res["symbol"], "entry_price": sig["entry"], "quantity": risk_data["quantity"]}))
...
async def tick_management():
    ...
    if active:
        ...
        async with state_lock: bot_state["active_trades"] = demo_execution.active_positions

async def tick_broadcaster():
    payload = {"type": "ACCOUNT_STREAM", "timestamp": int(datetime.now().timestamp() * 1000), "balance": bot_state["balance"], "equity": bot_state["equity"], "active_trades": bot_state["active_trades"], "is_running": bot_state["is_running"], "armed": bot_state["armed"], "mode": bot_state["mode"], "stats": bot_state["engine_stats"]}
    await manager.broadcast(json.dumps(payload))
    await data_engine.broadcast_market_update(bot_state["selected_market"])

# Loop wrappers
async def loop_wrapper(func, interval):
    while True:
        try: await func()
        except Exception as e: logger.error(f"Loop Error ({func.__name__}): {e}")
...
async def startup_event():
    logger.info("QUANTUM MASTER STARTING...")
    await broker_connector.initialize_from_db(db_manager)
    bot_state["latest_scan"] = await scanner_engine.scan_all()
    asyncio.create_task(loop_wrapper(tick_capital, 1))
    asyncio.create_task(loop_wrapper(tick_scanner, 5))
    asyncio.create_task(loop_wrapper(tick_management, 0.2))
    asyncio.create_task(loop_wrapper(tick_broadcaster, 0.2))
...
async def get_status(market_id: str = "btc_usdt"):
    ...
    return {"is_running": bot_state["is_running"], "mode": bot_state["mode"], "armed": bot_state["armed"], "balance": bot_state["balance"], "equity": bot_state["equity"], "selected_market": market_id, "asset_info": info, "ticker": ticker, "stats": bot_state["engine_stats"]}

@app.get("/api/history")
async def get_history(mode: str = "DEMO"): return db_manager.get_history(mode=mode)

@app.get("/api/scanner")
async def get_scanner(): return {"assets": bot_state["latest_scan"]}
...
async def save_settings(new_settings: Dict[str, str]):
    with db_manager._get_connection() as conn:
        for k, v in new_settings.items(): conn.execute("INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)", (k, v))
        conn.commit()
    return {"success": True}

@app.post("/api/start")
async def start_bot(): bot_state["is_running"] = True; return {"success": True}
@app.post("/api/stop")
async def stop_bot(): bot_state["is_running"] = False; return {"success": True}
@app.post("/api/arm")
async def arm_bot(): bot_state["armed"] = not bot_state["armed"]; return {"armed": bot_state["armed"]}
@app.post("/api/mode")
async def toggle_mode(): bot_state["mode"] = "REAL" if bot_state["mode"] == "DEMO" else "DEMO"; return {"mode": bot_state["mode"]}
@app.post("/api/emergency-stop")
async def emergency_stop_api(): await emergency_stop_logic(); return {"success": True}

async def emergency_stop_logic():
    bot_state["is_running"] = False; bot_state["armed"] = False
