    ...
    while True:
        try:
            ...
            async with state_lock:
                ...
                bot_state["engine_stats"]["markets"] = len(
                    data_engine.universe.get_all_ids())
                bot_state["engine_stats"]["scanned"] = len([r for r in results if r.get(
                    "status") != "ERROR" and r.get("status") != "DATA_UNAVAILABLE"])
                bot_state["engine_stats"]["analyzing"] = len(
                    [r for r in results if r.get("trend") != "NEUTRAL"])
                bot_state["engine_stats"]["signals"] = len(
                    [r for r in results if r.get("score", 0) >= 80])
                bot_state["engine_stats"]["tradable"] = len(
                    [r for r in results if r.get("tradable")])
            ...
            async with state_lock:
                ...
                with db_manager._get_connection() as conn:
                    row = conn.execute(
                        "SELECT value FROM settings WHERE key = 'max_open_positions'").fetchone()
                    max_pos = int(row["value"]) if row else 3

                if len(active_positions) < max_pos:
                    symbols_in_position = [p["symbol"]
                                           for p in active_positions]
                    candidates = sorted([r for r in results if r.get(
                        "score", 0) >= 80], key=lambda x: x.get("score", 0), reverse=True)
                        ...
                        if risk_data.get("allowed"):
                            ...
                            if exec_res.get("success"):
                                ...
                                db_manager.log_audit(
                                    "CRITICAL", "BACKGROUND_AUTO_TRADE", f"Auto-executed {mid} ({sig['score']}%)", exec_res)
    ...
    while True:
        try:
            async with state_lock:
                ...
                equity = balance + sum(t.get("pnl", 0.0)
            ...
            priority_mids = list(set(
                [selected, "btc_usdt", "eth_usdt", "sol_usdt", "aapl", "nvda", "eur_usd", "gold"]))
...
async def daily_report_loop():
    ...
    while True:
        try:
            ...
            if now.hour == 0:
                report = portfolio_engine.get_performance_report(
                    bot_state["mode"])
...
async def startup_event():
    db_manager.log_audit("INFO", "SYSTEM_STARTUP",
                         "Quantum Trade Pro Institutional Core Active.")
    ...
    with db_manager._get_connection() as conn:
        ...
        if float(settings.get("max_risk_pct", 0)) > 5.0:
            logger.warning(
                "CRITICAL: max_risk_pct is set > 5%. Resetting to 1.0% for safety.")
            conn.execute(
                "UPDATE settings SET value = '1.0' WHERE key = 'max_risk_pct'")
            conn.commit()

    db_manager.log_audit("INFO", "DATA_INTEGRITY",
                         "Data Authenticity Verified: Connected to Gate.io (LIVE).")
    ...
    with db_manager._get_connection() as conn:
        ...
        if "active_strategies" in settings:
            strategies = [s.strip()
                          for s in settings["active_strategies"].split(',')]
...
async def save_settings(new_settings: Dict[str, str]):
    ...
    if "max_risk_pct" in new_settings:
        risk_engine.max_risk_pct = float(new_settings["max_risk_pct"])
    if "max_leverage" in new_settings:
        risk_engine.max_leverage = int(new_settings["max_leverage"])
    if "max_daily_loss_pct" in new_settings:
        risk_engine.max_daily_loss_pct = float(
            new_settings["max_daily_loss_pct"])
    if "cool_down_mins" in new_settings:
        risk_engine.cool_down_mins = int(new_settings["cool_down_mins"])
    if "min_signal_score" in new_settings:
        signal_engine.min_score = int(new_settings["min_signal_score"])
    if "active_strategies" in new_settings:
        strategies = [s.strip()
                      for s in new_settings["active_strategies"].split(',')]
        signal_engine.set_active_strategies(strategies)

    if "telegram_token" in new_settings and "telegram_chat_id" in new_settings:
        notification_engine.update_config(
            new_settings["telegram_token"], new_settings["telegram_chat_id"])

    db_manager.log_audit("INFO", "SETTINGS_UPDATED",
                         "System parameters deployed to live engines.")
    return {"success": True}
...
async def list_brokers():
    with db_manager._get_connection() as conn:
        ...
        w_rows = conn.execute(
            "SELECT wallet_id as broker_id, provider as exchange_id, is_active, 'REAL' as mode FROM web3_wallets").fetchall()
...
async def toggle_broker(broker_id: str, active: bool):
    with db_manager._get_connection() as conn:
        conn.execute("UPDATE broker_configs SET is_active = ? WHERE broker_id = ?",
                     (1 if active else 0, broker_id))
        conn.commit()
    await broker_connector.initialize_from_db(db_manager)
    return {"success": True}
...
async def delete_broker(broker_id: str):
    with db_manager._get_connection() as conn:
        conn.execute(
            "DELETE FROM broker_configs WHERE broker_id = ?", (broker_id,))
        conn.commit()
...
async def get_audit_logs(limit: int = 20):
    with db_manager._get_connection() as conn:
        rows = conn.execute(
            "SELECT * FROM audit_logs ORDER BY timestamp DESC LIMIT ?", (limit,)).fetchall()
...
async def manual_trade(trade: Dict[str, Any]):
    ...
    # For now, just log it
    db_manager.log_audit("INFO", "MANUAL_TRADE_REQUEST",
                         f"Manual {trade.get('direction')} for {trade.get('market_id')}")
...
async def get_status(market_id: str = "btc_usdt"):
    ...
    if not ticker or df_ltf.empty:
        async with state_lock:
            ...
            balance = portfolio_engine.get_balance(
                bot_state["mode"])  # Fixed Lot 2
    ...
    analysis = analysis_engine.identify_structure(
        df_ltf, htf_bias=htf_analysis.get("trend"))
    ...
    async with state_lock:
        ...
        if current_mode == "REAL":
            ...
            balance = sum(b.get("total_usdt", 0.0)
                          for b in broker_balances.values() if "total_usdt" in b)
    ...
    signal_result = signal_engine.generate_signal(
        analysis, news_status, df_ltf, cross_quotes=cross_quotes, orderbook=orderbook, trades=trades)
    ...
    broker_valid = broker_info.get(
        "connected", False) or bot_state["mode"] == "DEMO"
    ...
    current_drawdown = ((risk_engine.peak_balance - balance) /
                        risk_engine.peak_balance * 100) if risk_engine.peak_balance > 0 else 0
    ...
    if signal_valid:
        ...
        leverage_valid = risk_data.get(
            "leverage", 0) <= risk_engine.max_leverage
    ...
    async with state_lock:
        ...
        if bot_state["is_running"] and state_machine.current_state != BotState.EMERGENCY_STOP:
            ...
            if alpha_override and technical_safety:
                ...
                db_manager.log_audit("CRITICAL", "ALPHA_OVERRIDE_TRADE",
                                     f"Executing High Confidence Signal ({signal_result['score']}%)", res)
                ...
                if can_execute_normal:
                    res = await execution_router.execute(bot_state["mode"], signal_result, risk_data, ticker)
                    db_manager.log_audit(
                        "INFO", "NORMAL_TRADE", f"Executing Normal Diagnostic Signal", res)

            db_manager.archive_signal(signal_result, "EXECUTED" if (
                alpha_override and technical_safety) else "FILTERED", diagnosis["main_blocker"])
        ...
        bot_state["equity"] = balance + \
            (active_trade["pnl"] if active_trade else 0)
    ...
    async with state_lock:
        ...
        return StatusResponse(
            ...
            stats=merged_stats,
            broker_info=broker_info,
            broker_connected=broker_info.get(
                "connected", False) or bot_state["mode"] == "DEMO",
            asset_info=info,
            best_setups=sorted([r for r in bot_state["latest_scan"] if r.get("score", 0) > 0 and r.get(
                "market_status") == "OPEN"], key=lambda x: x["score"], reverse=True)[:5]
        )
...
async def set_demo_balance(amount: float):
    ...
    db_manager.log_audit("INFO", "BALANCE_UPDATE",
                         f"Demo balance set to {amount}")
...
async def emergency_stop():
    async with state_lock:
        ...
        db_manager.log_audit("ERROR", "EMERGENCY_STOP",
                             "All positions force closed locally and on brokers.")

        # Notification (Lot 10)
        asyncio.create_task(notification_engine.notify(
            "EMERGENCY_STOP", {"reason": "User Triggered"}))
...
async def arm_bot():
    async with state_lock:
        ...
        db_manager.log_audit("INFO", "SYSTEM_ARM",
                             f"System armed state: {bot_state['armed']}")
